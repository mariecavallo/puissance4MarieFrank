...
public class StrategyMarie2 extends Strategy {
    ...
    public Integer choixColonneNiveau(Board board, Color myColor, Color colorAdverse) {
        ...
        if (choixColonne == null || colonnePleine(board, choixColonne)) {
            ...
            if (choixColonne == null || colonnePleine(board, choixColonne)) {
                ...
                if (choixColonne == null || colonnePleine(board, choixColonne) || pionFaisantGagner(board, myColor, colorAdverse, choixColonne)) {
                    choixColonne = verifColonneDefense(board, colorAdverse, 2);
                    if ((choixColonne == null || colonnePleine(board, choixColonne) || pionFaisantGagner(board, myColor, colorAdverse, choixColonne)) && board.getJetons()[5][3].getColor().equals(Color.TRANSPARENT)) {
                        choixColonne = 3;
                    } else {
    ...
    public boolean besoinDefendre(Board board, Color colorAdverse) {

        boolean defense = false;

        for (int j = 0; j < 7; j++) {
            int ligneDescente = descentePion(board, j);
            if (board.compteurVertical(colorAdverse, ligneDescente, j) == 3) {
                defense = true;
            }
            if (board.compteurHorizontal(colorAdverse, ligneDescente, j) == 3) {
                defense = true;
            }
            if (board.compteurDiagonaleSONE(colorAdverse, ligneDescente, j) == 3) {
                defense = true;
            }
            if (board.compteurDiagonaleNOSE(colorAdverse, ligneDescente, j) == 3) {
                defense = true;
            }
        }
        return defense;
    ...
    public boolean pionFaisantGagner(Board board, Color myColor, Color colorAdverse, int colonneChoisi) {
        boolean pionFaisantGagner = false;
        board.getJetons()[descentePion(board, colonneChoisi)][colonneChoisi].setColor(myColor);
        if (besoinDefendre(board, colorAdverse)) {
            pionFaisantGagner = true;
        } else {
            pionFaisantGagner = false;
        }
        board.getJetons()[descentePion(board, colonneChoisi)][colonneChoisi].setColor(Color.TRANSPARENT);
        return pionFaisantGagner;
    }
}
